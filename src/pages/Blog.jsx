  ...
  return (
    <>
      ...
      <section className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 max-w-7xl mx-auto px-5 mb-10">
        {stories.map((story) => (
          <Link to={`/blog/${story.slug.current}`} key={story.slug.current}>
            <article className="border border-slate-400 dark:border-slate-100 rounded-lg overflow-hidden hover:bg-slate-200 dark:hover:bg-slate-800 transition-all duration-200">
              {story.mainImage && (
                <img
      ...
      <div className="max-w-7xl mx-auto px-5 mb-20 flex items-end justify-end">
        <Link
          to="/"
          className="bg-white dark:bg-slate-100 dark:hover:bg-slate-500 py-2 px-8 rounded shadow text-slate-800 dark:text-slate-800 tracking-wide hover:opacity-75 transition-all duration-200"
        >
          Back to Homepage
        </Link>
      </div>
